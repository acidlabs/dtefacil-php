...
define('DF_REST_GET', 'GET');
define('DF_REST_POST', 'POST');
define('DF_REST_PUT', 'PUT');
define('DF_REST_DELETE', 'DELETE');
define('DF_REST_SOCKET_TIMEOUT', 1);

class DF_REST_TransportFactory {
    function get_available_transport($requires_ssl, $log) { 
        if(@DF_REST_CurlTransport::is_available($requires_ssl)) {
            return new DF_REST_CurlTransport($log);
        } else if(@DF_REST_SocketTransport::is_available($requires_ssl)) {
            return new DF_REST_SocketTransport($log);
        } else { 
            $log->log_message('No transport is available', DF_REST_LOG_ERROR);
    ...
    function split_and_inflate($response, $may_be_compressed, $log) {        
        ...
        if($may_be_compressed && preg_match('/^Content-Encoding:\s+gzip\s+$/im', $headers)) {        
            ...
            $log->log_message('Inflated gzipped response: '.$original_length.' bytes ->'.
                strlen($result).' bytes', get_class(), DF_REST_LOG_VERBOSE);
        }
 ...
 * @since 1.0
 */
class DF_REST_CurlTransport {

    var $_log;
    var $_curl_zlib;

    function DF_REST_CurlTransport($log) {
        $this->_log = $log;
    ...
    function make_call($call_options) {
        ...
        switch($call_options['method']) {
            case DF_REST_PUT:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, DF_REST_PUT);
            ...
            case DF_REST_POST:
                ...
                break;
            case DF_REST_DELETE:
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, DF_REST_DELETE);
                break;
        }
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making request with curl_error: '.curl_errno($ch),
                get_class($this), DF_REST_LOG_ERROR);
            trigger_error('Error making request with curl_error: '.curl_error($ch), E_USER_ERROR);
        }
        
        list( $headers, $result ) = @DF_REST_TransportFactory::split_and_inflate(
...
class DF_REST_SocketWrapper {
    var $socket;

    function open($domain, $port) {
        $this->socket = fsockopen($domain, $port, $errno, $errstr, DF_REST_SOCKET_TIMEOUT);

        if(!$this->socket) {
            die('Error making request with '.$errno.': '.$errstr);
            return false;
        } else if(function_exists('stream_set_timeout')) {
            stream_set_timeout($this->socket, DF_REST_SOCKET_TIMEOUT);
        }
...
class DF_REST_SocketTransport {

    var $_log;
    var $_socket_wrapper;

    function DF_REST_SocketTransport($log, $socket_wrapper = NULL) {
        $this->_log = $log;

        if(is_null($socket_wrapper)) {
            $socket_wrapper = new DF_REST_SocketWrapper();
        }
