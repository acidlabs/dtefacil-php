...
class DF_REST_SerialiserFactory {
    function get_available_serialiser($log) {
        $log->log_message('Getting serialiser', get_class($this), DF_REST_LOG_VERBOSE);
        if(@DF_REST_NativeJsonSerialiser::is_available()) {
            return new DF_REST_NativeJsonSerialiser($log);
        } else {
            return new DF_REST_ServicesJsonSerialiser($log);
        }
    }
    ...
    function check_encoding($data) {
        foreach($data as $k => $v) {
            ...
            if(is_array($v)) {
                $data[$k] = @DF_REST_SerialiserFactory::check_encoding($v);
...
class DF_REST_NativeJsonSerialiser {

    var $_log;

    function DF_REST_NativeJsonSerialiser($log) {
        $this->_log = $log;
    }
    ...
    function serialise($data) {
        return json_encode(@DF_REST_SerialiserFactory::check_encoding($data));
    }
...
class DF_REST_ServicesJsonSerialiser {
    
    var $_log;
    var $_serialiser;
    
    function DF_REST_ServicesJsonSerialiser($log) {
        $this->_log = $log;
    ...
    function serialise($data) {
        return $this->_serialiser->encode(@DF_REST_SerialiserFactory::check_encoding($data));
    }
